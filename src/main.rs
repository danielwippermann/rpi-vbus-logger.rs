...
use resol_vbus::*;
use resol_vbus::chrono::{DateTime, TimeZone, UTC};
...
mod config;
use config::{Config, DatabaseConfig, FieldConfig, read_config};

mod data_set_stability;
use data_set_stability::DataSetStability;
...
fn process_fields_config<'a>(field_configs: &'a Vec<FieldConfig>, spec: &'a Specification, db_name: &str, db_conn: &mut PooledConn) -> Result<Vec<Field<'a>>> {
    let mut fields = Vec::with_capacity(field_configs.len());

    for fc in field_configs.iter() {
        let column_is_valid = fc.column.chars().all(|c| {
            match c {
                '0'...'9' | 'A'...'Z' | 'a'...'z' | '_' => true,
                _ => false,
            }
        ...
        if !column_is_valid {
            return Err(Error::from(format!("Invalid chars in column name {:?}", fc.column)));
        }
        match fc.column.as_str() {
            "id" | "timestamp" => return Err(Error::from(format!("Reserved column name used: {:?}", fc.column))),
            _ => (),
        }

        let row_count = db_conn.first_exec("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = :schema AND TABLE_NAME = :table AND COLUMN_NAME = :column", (&db_name, "data", &fc.column))
            .expect("Unable to get column information")
            .expect("Expected to get row count but got nothing");
        match row_count [0] {
            Value::Int(1) => (),
            Value::Int(0) => return Err(Error::from(format!("Unknown column name used: {:?}", fc.column))),
            _ => return Err(Error::from(format!("Unexpected row_count: {:?}", row_count))),
        };
        ...
        let field_spec_pos = match packet_spec.get_field_spec_position(&fc.field_id) {
            Some(p) => p,
            None => return Err(Error::from(format!("Unknonwn field ID {:?} for packet ID {:?}", fc.field_id, fc.packet_id))),
        };
...
fn main() {
    ...
    let mut last_timestamp = match pool.first_exec("SELECT MAX(timestamp) FROM data", ()).expect("Unable to fetch max timestamp") {
        Some(row) => {
            match row [0] {
                Value::NULL => UTC::now(),
                Value::Date(year, month, day, hour, minute, seconds, micros) => {
                    UTC.ymd(year as i32, month as u32, day as u32).and_hms_micro(hour as u32, minute as u32, seconds as u32, micros)
                },
    ...
    let mut data_set_stability_counter = 0;
    ...
    while let Some(data) = ldr.read_data().expect("Unable to read data from VBus") {
        ...
        let length_before = data_set.len();

        data_set.add_data(data);

        let length_after = data_set.len();

        if length_before != length_after {
            println!("Data set changed, waiting for it to stabilize again...");

            data_set_is_stable = false;
            data_set_stability_counter = 0;
        } else if data_set_is_stable {
            // nop
        } else if data_set_stability_counter >= (length_after * 3) {
            println!("Data set stabilized");

            data_set_is_stable = true;
            data_set.sort();

            print_fields(&spec, &data_set);
        } else {
            println!("Data set stabilizing at {}%", data_set_stability_counter * 33 / length_after);
            data_set_stability_counter += 1;
        }

        if !data_set_is_stable {
            continue
        }

        if last_timestamp.timestamp() / logger_interval != data_timestamp.timestamp() / logger_interval {
            last_timestamp = data_timestamp;

            println!("Storing data set for timestamp {}", &last_timestamp);

            let sql_timestamp = mysql::chrono::NaiveDateTime::from_timestamp(last_timestamp.timestamp(), 0);

            db_conn.first_exec("INSERT INTO data(timestamp) VALUES(:timestamp)", (sql_timestamp, )).expect("Unable to insert timestamp");

            let result = db_conn.first_exec("SELECT LAST_INSERT_ID()", ()).expect("Unable to get insert ID").expect("Expected last insert ID row but got nothing");
            let id = &result [0];

            for data in data_set.as_data_slice() {
                ...
                for field in fields.iter() {
                    if field.packet_id == packet_id {
                        ...
                        let field_spec = field.packet_spec.get_field_spec_by_position(field.field_spec_pos);
                        let raw_value = field_spec.raw_value_f64(frame_data);
                        // println!("         raw_value = {:?}", raw_value);
                        db_conn.first_exec(&field.stmt, (raw_value, id)).expect(&format!("Unable to update column {}", field.column));
                    }
                }
            }
        }
